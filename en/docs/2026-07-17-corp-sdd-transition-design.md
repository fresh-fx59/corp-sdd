...
> **AMENDMENTS 2026-08-04/05** (operator decisions, each with a tested runbook in `docs/`):
> **(1)** Zoekt moved **Phase 2 → Phase 0** — see §13 item 1.
> **(2)** `openspec` on npm is a placeholder at 0.0.0; the real CLI is **`@fission-ai/openspec`**. OpenSpec's root walk does **not** stop at a `.git` boundary, so every code repo must be onboarded and clones must stay outside the store — [2026-08-04-openspec-root-resolution-fix.md](2026-08-04-openspec-root-resolution-fix.md).
> **(3)** Cross-repo stories: **the parent story is the store-contract ticket**, one child ticket per repo, delivered by **one extended `corp-spec`** (no separate fan-out verb) with a confirmation gate — [2026-08-04-corp-sdd-cross-repo-fanout.md](2026-08-04-corp-sdd-cross-repo-fanout.md). This supersedes harness-pack §D steps 1–4.
> **(4)** Branch/commit conventions are now enforced mechanically (`feature/ABCD-1234`, `feat(ABCD-1234): text`), and the split-brain lint the design called for is **built** — [2026-08-04-corp-sdd-setup-task-for-agent.md](2026-08-04-corp-sdd-setup-task-for-agent.md).
...
**Top 3:**
1. **Zoekt** (Apache-2.0) — trigram cross-repo search (two small Go services + mirror cron); pairs with the existing LSP MCP for symbol-precise follow-up. Largest agent-quality delta on a multi-module JVM estate. *~~Enters: Phase 2, when cross-repo work starts.~~* **AMENDED 2026-08-04 — enters Phase 0** (operator decision): the generated `openspec/index.md` is spec-derived and holds 0 capabilities on a brownfield repo, so until living specs accumulate, cross-repo code search **is** the agent's map of the estate. Tested runbook: [2026-08-04-corp-sdd-zoekt-setup.md](2026-08-04-corp-sdd-zoekt-setup.md).
